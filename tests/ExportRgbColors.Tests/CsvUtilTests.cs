...
namespace ExportRgbColors.Tests
{
    ...
    {
        [Fact]
        public void Header_Is_ColorIndex_And_RGB_Together()
        {
            Assert.Equal(new[] { "ColorIndex", "RGB", "R", "G", "B", "Layer" }, CsvUtil.Header);
        }
        ...
        [Fact]
        public void ByLevelRow_Uses_ColorIndex_Minus_One_And_RGB()
        {
            string line = CsvUtil.FormatRow(CsvUtil.ByLevelRow("EQPM", 0, 0, 255));
            Assert.Equal("-1,\"0, 0, 255\",0,0,255,EQPM", line);
        }
        ...
        public void Shared_Color_Writes_One_ByLevel_Row_Per_Level()
        {
            ...
            Assert.Equal("1,\"0, 0, 255\",0,0,255,", lines[0]);
            Assert.Equal("-1,\"0, 0, 255\",0,0,255,EQPM", lines[1]);
            Assert.Equal("-1,\"0, 0, 255\",0,0,255,R-LITE", lines[2]);
        }
        ...
        public void Layer_With_Comma_Is_Quoted()
        {
            ...
            Assert.Equal("-1,\"255, 0, 0\",255,0,0,\"R-LITE, EQPM\"", line);
        }

        [Fact]
        public void Write_Produces_Bom_Utf8_Csv_With_Index_And_Rgb()
        {
            ...
            try
            {
                ...
                Assert.Equal("-1,\"0, 0, 255\",0,0,255,Default", lines[2]);
            }
            finally
            {
                if (File.Exists(path))
